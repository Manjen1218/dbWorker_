...
def mcc_process(
    ...
    mcc_null_conditions = [f"(`{field}` is NULL OR `{field}` = 'NA')" for field in mcc_field_names]
    where_clause = " AND ".join(mcc_null_conditions)
    sql = f"SELECT fullpath FROM `{tablename}` WHERE ({where_clause} AND `{time_column}` >= %s) OR (mcc_check != 'NA' and mcc_check_time = '' and mcc_upload_time = '')"
...
def process_sku_with_comprehensive_cleanup(
    ...
    class TransactionManager:
        def __init__(self, db_helper: MySQLHelper, db_name: str, lock_timeout: int = 5, logger: Logger = None):
            self.db = db_helper
            self.dbname = db_name
        ...
        def __enter__(self):
            try:
                ...
                if not self.db.use_database_lock(self.dbname):
                    ...
                    raise TransactionTimeoutException(f"Could not select database {self.dbname}")
                ...
                if self.logg:
                    self.logg.log_info(f"Started transaction on {self.dbname}")

            except Exception as e:
                raise TransactionTimeoutException(f"Unexpected error when starting transaction: {e}")
            ...
            if self.in_transaction:
                try:
                    if exc_type is None:
                        ...
                        if self.logg:
                            self.logg.log_info(f"Transaction committed on {self.dbname}")
                    else:
                        self.db.execute_query("ROLLBACK")
                        if self.logg:
                            self.logg.log_warn(f"Transaction rolled back on {self.dbname} due to error: {exc_val}")
                except Exception as e:
                    if self.logg:
                        self.logg.log_error(f"Error finalizing transaction: {e}")
                finally:
                    self.in_transaction = False
    ...
    for test_station, ts_info in sku_info['test_station'].items():
        tablename = ts_info['tablename']
        try:
            with TransactionManager(db, dbname, lock_timeout=5, logger=logg):
            ...
            logg.log_warn(f"Skipped table <{tablename}> due to transaction timeout")
            continue
